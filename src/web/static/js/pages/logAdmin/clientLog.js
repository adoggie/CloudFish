...
$(function() {
    ...
    var pager = $('#clientLog').datagrid('getPager');
    $(pager).pagination({
...
var getData = function (pageNumber, pageSize) {
    ...
    $.ajax({
        type: "post",
        url:'/webapi/admin/getAppUserLog/',
        data:data,
        dataType: "json",
        success: function (data) {
            if (data.status == 0) {
                $('#clientLog').datagrid('loadData', {total:data.pgctl.total,rows:data.result});
            } else {
...
function exportAdminOperationLog(){
    ...
    window.open(sy.data.url+'/webapi/admin/exportAppUserLog/?case='+export_case);
