...
var erweima_scale; // add by wan@20141103
...
var IS_FA = "false";//wan //2015.09.15是否对接中间件，true 对接 false 不对接 字符串
var RenderingStates = {
  INITIAL: 0,
  RUNNING: 1,
  PAUSED: 2,
...
var par;//add by dj@20151012
var fileParams;//add by dj@20151012
/**
 * 获取url中的参数
 */
function getUrlParam(name){
  var reg = new RegExp("(^|&)" + name + "=([^&]*)(&|$)"); //构造一个含有目标参数的正则表达式对象
  var r = fileParams.match(reg);  //匹配目标参数
  alert(r)
  if(r != null){
    return unescape(r[2]);
  }else{
...
// add by wan@20150128
function dec_print_count(urlstr)
{
  var param = null;
  if(IS_FA=="true"){
    param =  urlstr.split("/application/decrypt/?")[1];
  }else{
    param =  urlstr.split("/file/previewAttachment/?")[1];
  }

  var file_id = param.split("&")[0];
  file_id = file_id.split("=")[1];
  //alert(file_id);

  var arc_id = param.split("&")[1];
  arc_id = arc_id.split("=")[1];
  //alert(arc_id);

  var status = param.split("&")[2];
  ...
  //alert("webpage_send_cmd " + ", status = " + status + ", arc_id = " + arc_id+ ", file_id = " + file_id);

  var urlpath = urlstr.split('/static/js')[0];
  ...
  $.ajax({
    async: false,
    type: "post",
    url: urlpath + "/webapi/ras/print_archive/",
    data:{
      print_info:data
    },
    dataType: "json",
    ...
    success: function (data) {
      if (data.status == 0) {
        alert("服务器成功减少打印次数(1次).")
      } else {
        var message = '错误代码:' + data.errcode + ', ' + data.errmsg + '!';
        alert('服务器减少打印次数失败:'+ message);
      }
    }
...
function webpage_send_cmd(cmdtype, urlstr)
{
  //alert("cmdtype " + cmdtype);
  urlstr = unescape(urlstr);
  //alert("urlstr " + urlstr);

  if( cmdtype == 1)
    dec_print_count(urlstr);
  else if( cmdtype == 2)
    get_erweima();
  else alert("未知命令: " + cmdtype);
...
function encoder()
{
  try{
    var string = '';
    for (var i = 0; i < documentArrayBuffer.length; i++) {
      string += String.fromCharCode(documentArrayBuffer[i]);
    }

    documentStr = btoa(string);
  }
  catch(e)
  {
    PDFView.error(mozL10n.get('loading_error', null,
        'An error occurred while load the PDF.'), e);
  }
}
...
var SecondaryToolbar = {
  ...
  initialize: function secondaryToolbarInitialize(options) {
    ...
    var elements = [
      ...
      { element: this.printPreview, handler: this.printPreviewClick },
  ...
  printPreviewClick: function secondaryToolbarPrintPreviewClick(evt) {
    // add by wan@20140923
    try{
      if(windowex.printPreview && typeof(windowex.printPreview ) == "function") {
        if(documentStr == ''){
          encoder();
        }

        windowex.printPreview(documentStr);
      }
      else {
        var printPreviewMessage = mozL10n.get('printing_preview_not_supported', null,
            'Warning: This browser is not support printing preview.');
        PDFView.error(printPreviewMessage);
      }
    }
    catch(e) {
      var printPreviewMessage = mozL10n.get('printing_preview_not_supported', null,
  ...
  printClick: function secondaryToolbarPrintClick(evt) {
    // comment by wan@20140923
    //window.print();

    try{
      if(windowex.print && typeof(windowex.print ) == "function") {
        if(documentStr == ''){
          encoder();
        }

        windowex.print(documentStr);
      }
      else {
        //add by dj@20151012
        //var printMessage = mozL10n.get('printing_not_supported', null,
        //          'Warning: This browser is not support printing.');
        //PDFView.error(printMessage);
        var params = null;
        var urlstr = fileParams;

        if(IS_FA=="true"){
          params =  urlstr.split("/application/decrypt/?")[1];
        }else{
          params =  urlstr.split("/file/previewAttachment/?")[1];
        }
        var file_id = params.split("&")[0];
        file_id = file_id.split("=")[1];


        var arc_id = params.split("&")[1];
        arc_id = arc_id.split("=")[1];
        ...
        var userToken =  params.split("&")[3];
        userToken = userToken.substr(11);
        ...
        var localhttp_url =  params.split("&")[5];
        localhttp_url = localhttp_url.split("=")[1];


        var codeType =  params.split("&")[6];
        codeType = codeType.split("=")[1];


        var codePosition =  params.split("&")[7];
        codePosition = codePosition.split("=")[1];


        var codeStr =  params.split("&")[8];
        codeStr = codeStr.split("=")[1];
        codeStr = unescape(codeStr);//unescape解码


        var printControl =  params.split("&")[9];
        printControl = printControl.split("=")[1];
        //var printMessage = mozL10n.get('printing_not_supported', null,
        //    'Warning: This browser is not support printing.');
        //PDFView.error(urlstr);
        ...
        var viewUrl = localhttp_url + "/application/printpdf/";
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data.status == 0) {
            var count = data.result.count;
            var logurl = window.location.protocol +"//"+ window.location.host+"/webapi/ras/createPrintLog/";
            var printdata ={
              user_token :userToken,
              arc_id:arc_id,
              file_id:file_id,
...
var PDFView = {
  ...
  initialize: function pdfViewInitialize() {
    ...
    SecondaryToolbar.initialize({
      ...
      printPreview: document.getElementById('secondaryPrintPreview'),
  ...
  setScale: function pdfViewSetScale(value, resetAutoSettings, noScroll) {
    erweima_scale = value;  // add by wan@2014110
    if (value === 'custom') {
      return;
    }
  ...
  get supportsPrinting() {
    ...
    var value = 0;
    try{
      if(windowex.isOpenBrowser && typeof(windowex.isOpenBrowser) == "function"){
        value = windowex.isOpenBrowser();
      }else{
        //alert("supportsPrinting:info: windowex.isOpenBrowser() not function.");
      }
    }catch(e){
      //alert("supportsPrinting:exception: windowex.isOpenBrowser() not exist.");
    }
...
function webViewerLoad(evt) {
  ...
  /*
   ...
   document.body.onpaste= document.body.oncontextmenu=function(){
   return false;
   };
  ...
  document.body.oncopy= document.body.oncontextmenu=function(){
    return false;
  };
}
...
function webViewerInitialized() {
  ...
  par = document.location.search.substring(1);// add by dj@20151012
  var file = 'file' in params ? params.file : DEFAULT_URL;
  fileParams = file
  IS_FA = 'fa' in params ? params.fa : "false";
  ...
  if (!window.File || !window.FileReader || !window.FileList || !window.Blob) {
    ...
    //add by dj@20151012
    document.getElementById('secondaryPrintPreview').setAttribute('hidden', 'true');
    document.getElementById('printPreview').setAttribute('hidden', 'true');
    ...
    document.getElementById('viewOutline').setAttribute('hidden', 'true');
    document.getElementById('viewAttachments').setAttribute('hidden', 'true');
    document.getElementById('viewThumbnail').setAttribute('hidden', 'true');
  }
  ...
  locale = 'zh-CN';   // add by wan@20141103 
  ...
  if (!PDFView.supportsPrinting) {
    /*
     ...
     document.getElementById('secondaryPrintPreview').classList.add('hidden');
     */
  }
  ...
  document.getElementById('printPreview').addEventListener('click',
      SecondaryToolbar.printPreviewClick.bind(SecondaryToolbar));
